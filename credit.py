# istenen input 16, 15, ya da 13 basamaklı bir kart numarası olmalıdır, 
...
count = len(number) #sayının kaç basamaklı olduğu hesaplanır.
first_two = int(number[0: 2]) #sayının son iki rakamı alınır - alternatif : last_two = number // 10 ** (count -2)
number = int(number)
sum = 0
...
while number:
    ...
    double_of_even_digit = (number % 10) * 2
    sum += double_of_even_digit % 10 + double_of_even_digit // 10  #rakamları toplanarak sum'a eklenir.
